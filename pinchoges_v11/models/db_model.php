...
/*****************ELIMINAR PINCHOS***************************/
	public function eliminarPinchos(){
		...
		$login=$_SESSION['login'];
		$sql="select * from pincho where pincho_validado = true order by nombre_pincho";
		$pinchos=array();
		$consulta = mysql_query($sql);
...
/*****************MODIFICA LA VALORACIÓN DEL PINCHOS********/
  ...
  /************Desasignar Pinchos listar los Jurados Profesionales y número de asignaciones*/
	...
	/*----------------------------------------------------------*/
	/*---------------------------------------------------------*/
	/**************Elimiar Jurado Profesional******************/
	public function eliminarJurPro(){	
		
		$_SESSION['errorSQL'] = 1;
		
			$sql="DELETE FROM jurado_profesional  
				WHERE
				nombre_jurPro ='".$_SESSION['nombreJurPro']."'";
			mysql_query($sql);
		
		//Validamos que la eliminación se ha realizado correctamente
		if (mysql_affected_rows() > 0)
		{
			$_SESSION['errorSQL'] = 0;
		}
	...
	public function gestionarAsignaciones(){	
	
		
		//Recuperamos datos de las asignaciones realizadas para mostrar
		//en el inicio de este submenú
		
		$sql="SELECT P.nombre_pincho, JP.nombre_jurPro
		      FROM asigna_pincho A, jurado_profesional JP, pincho P
		      WHERE A.ID_pincho = P.ID_pincho
			  AND A.ID_jurPro = JP.ID_juradoProfesional
			  ORDER BY JP.nombre_jurPro";
		
		$resultado = mysql_query($sql);
		
		
		//Validamos que la consulta  se ha realizado correctamente
		...
		$_SESSION['asignaciones']=$asignaciones;

	}
	
	/*----------------------------------------------------------*/
	/*---------------------------------------------------------*/
	/********Comprobar asignacines de profesional****************/
	public function checkAsignacionesPro(){	
	
	  $sql="select ID_juradoProfesional 
	      from asigna_pincho, jurado_profesional 
	      where asigna_pincho.ID_jurPro = jurado_profesional.ID_juradoProfesional
	      and
	      nombre_jurPro='".$_SESSION['login']."'
	      ";
	    $resultado = mysql_query($sql);
	    
	    if($row=mysql_fetch_row($resultado)){
	      $_SESSION['inicio_JP']=1;
	    }else
	      $_SESSION['inicio_JP']=0;
